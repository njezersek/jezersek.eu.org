...
<html lang="en">
 ...
 <script>
    ...
    function init() {
		...
		cube = new THREE.Mesh(new THREE.CubeGeometry(50, 25, 80), mat);
		...
		cube1 = new THREE.Mesh(new THREE.CubeGeometry(20, 20, 20), mat1);
		cube1.rotation.x = Math.PI * 0.1;
		cube1.position.set(-400, -250, -120);
		scene.add(cube1);
		...
		krneki = 0;
		// cubes instead particles
		var matc = new THREE.MeshLambertMaterial({
			wireframe: false,
			//opacity: 0.1,
			//color: 'blue',
			transparent: true
		...
		for (var x = 0; x<20; x++){
			for (var y = 0; y<12; y++){
				for (var z = 0; z<6; z++){
					cubes[krneki] = new THREE.Mesh(new THREE.CubeGeometry(30, 30, 30), 
					                               new THREE.MeshLambertMaterial( { color: Math.random() * 0xffffff, transparent: true, opacity: 0.1 } ) );
					cubes[krneki].rotation.x = Math.PI * 0.1;
					cubes[krneki].position.set(x*30-200, y*30-120, z*30-60);			
					scene.add(cubes[krneki]);	
					krneki++;
				}	
			}
		}

		cube2 = new THREE.Mesh(new THREE.CubeGeometry(20, 20, 20), mat1);
		cube2.rotation.x = Math.PI * 0.1;
		cube2.position.set(-300, -150, -60);
		scene.add(cube2);
	...
	function animate() {
		...
		camera.position.x = radius * Math.sin( THREE.Math.degToRad( mouseX ) );
		camera.position.y = radius * Math.sin( THREE.Math.degToRad( mouseY ) );
		...
		//cube1.rotation.set(-mouseY/200 + 1, -mouseX/200, 0);		
		//cube2.rotation.set(-mouseY/200 + 1, -mouseX/200, 0);		
		//for (var i = 0; i<1440; i++){
		//	cubes[i].rotation.set(-mouseY/200 + 1, -mouseX/200, 0);
		//}
